package softwaretechnik_uebung8_2;
...
public class AuthorListParser {
    ...
    private static final  Set<String> avoidTermsInLowerCase = Set.of(
    ...
    private static final int offsetTokenName = 2; 
     ...
     *
     * @param indexArray and nameList the List containing the names
    ...
    private Optional<Author> getauthor() {
        ...
        while (continueLoop) {
            ...
            switch (token) {
                ...
                case WORD:
                    ...
                    if (vonStart < 0) {
                        if (!tokenCase) {
                            int previousTermToken = (tokens.size() - TOKEN_GROUP_LENGTH - TOKEN_GROUP_LENGTH) + offsetTokenName;
        ...
        if (commaFirst < 0) { // no commas
            if (vonStart < 0) { // no 'von part'
                ...
                int index = (tokens.size() - (2 * TOKEN_GROUP_LENGTH)) + offsetTokenName;
                if (index > 0) {
    ...
    private String concatTokens(List<Object> tokens, int start, int end, int offset, boolean dotAfter) {
        ...
        while (updatedStart < end) {
            result.append(tokens.get((updatedStart - TOKEN_GROUP_LENGTH) + offsetTokenName));
